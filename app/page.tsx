...
export default function Home() {
  ...
  return (
    <div className="space-y-8">
      <header className="card bg-blue-50 border-blue-100">
        <h1 className="text-3xl font-bold">AidAtlas AI</h1>
        <p className="mt-1 text-slate-600">AI social-good resource navigator for scholarships, emergency aid, mental-health support, and job pathways.</p>
      </header>
      <section className="grid gap-6 md:grid-cols-[1fr_280px]">
        <div className="card space-y-4">
          <h2 className="text-xl font-semibold">Guided Intake</h2>
          {(['fullName', 'location', 'ageStatus'] as const).map((k) => (
            <div key={k}>
              <label className="text-sm font-medium">{fieldLabels[k]}</label>
              <input className="input" placeholder={k === 'fullName' ? 'Jane Doe' : k === 'location' ? 'City, State' : '22, college student'} value={form[k]} onChange={(e) => setForm({ ...form, [k]: e.target.value })} />
            </div>
          ))}
          <div>
          ...
          {(['goals', 'situation'] as const).map((k) => (
            <div key={k}>
              <label className="text-sm font-medium">{fieldLabels[k]}</label>
              <textarea className={`input ${k === 'goals' ? 'min-h-20' : 'min-h-24'}`} placeholder={k === 'goals' ? 'Get food support and short-term rent help' : 'Describe your current situation'} value={form[k]} onChange={(e) => setForm({ ...form, [k]: e.target.value })} />
            </div>
          ))}
