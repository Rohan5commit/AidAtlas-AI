  ...
  return (
    <div className="space-y-6">
      <section className="card">
        ...
        <p className="mt-2 text-sm text-slate-500">{plan.disclaimer}</p>
      </section>
      ...
      <section className="card">
        ...
        <ul className="list-disc pl-5 text-sm">{plan.followUpPlan.map((s) => <li key={s}>{s}</li>)}
        </ul>

        <h2 className="mt-4 font-semibold">Progress Tracker</h2>
        <ul className="mt-2 space-y-1">{plan.progress.map((p) => <li key={p.label}>{p.done ? '\u2705' : '\u25A1'} {p.label}</li>)}
        </ul>
      </section>
    </div>
  );
